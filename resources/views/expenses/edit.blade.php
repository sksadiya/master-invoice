  ...
  <div class="col-xxl-12">
    ...
    <div class="card-header">
      ...
      <div class="card-body p-4">
        <form action="{{ route('expense.update',$expense->id)}}" method="post" id="expense-create-form" name="expense-create-form"
          ...
          <div class="row">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('category') is-invalid @enderror" aria-placeholder="Expense Category" id="category" name="category">
                        @foreach ($categories as $category)
                            <option {{ $expense->expense_category_id == $category->id ? 'selected' : ''}} value="{{ $category->id}}">{{ $category->name }}</option>
                        @endforeach
                    </select>
                    @error('category')
                    <div class="invalid-feedback">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('member') is-invalid @enderror" id="member" aria-placeholder="Team Member" name="member">
                        @foreach ($members as $member)
                            <option {{$expense->team_member_id == $member->id ? 'selected' : ''}} value="{{ $member->id}}">{{ $member->name }}</option>
                        @endforeach
                    </select>
                    @error('member')
                    <div class="invalid-feedback">
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="number" value="{{ $expense->amount }}" placeholder="eg. 100" class="form-control @error('amount') is-invalid @enderror" min="0" step="" name="amount" id="amount">
                    @error('amount')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="text" value="{{ $expense->title}}" placeholder="Title" class="form-control @error('title') is-invalid @enderror" name="title" id="title">
                    @error('title')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    @endif
                    <div class="mt-2">
                        <label>Uploaded Bill Receipt:</label>
                        <p>{{ $expense->bill_file }}</p>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <textarea rows="3" class="form-control @error('description') is-invalid @enderror" name="description" id="description" placeholder="Description">
                    {{ $expense->description }}
                    </textarea> 
                    @error('description')
                    <div class="invalid-feedback">
